...
interface Props {
    ...
    dataSection: ArticlesCardType[]
    seccion: string
}

export default function FullArticle({data, seccion, dataSection}: Props){
        ...
        <section>
            {
                ...
                ? <>
                    ...
                    <article className="py-4 flex flex-col gap-4 w-3/4 mx-auto max-sm:w-full">
                        ...
                        <p className="text-secondaryText lg:text-xs max-sm:text-xs">{data.date}</p>
                        ...
                        <h4 className="font-semibold text-lg w-3/4 mx-auto max-sm:w-full">Mas sobre el tema</h4>
                    <article className="border-t-2 grid grid-cols-5 text-center gap-6 border-black py-4 max-sm:grid-cols-3 w-3/4 mx-auto max-sm:w-full">
                        {
                            filterNota.map(e => {
                                return <RelationCard key={e._id} img={e.img} title={e.title} section={e.section} _id={e._id}/>
                            })
                        }
                    </article>
                </>
