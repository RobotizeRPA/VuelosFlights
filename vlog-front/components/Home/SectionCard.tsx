...
export default function SectionCardHome({section, data}: Props){
    ...
    return(
        ...
        <h4 className="grid col-span-3 font-bold text-2xl border-b-2 border-b-primaryColor max-sm:col-span-2">{sectioon}</h4>
        {
            data.map(e => {
                return <ArticleCard key={e._id} title={e.title} section={e.section} img={e.img} _id={e._id}/>
            })
        }
        <div className="flex justify-center items-center h-[210px]"><Link href={`/${section}`} className="w-3/6 mx-auto text-center p-4 bg-primaryColor text-white">Ver todos</Link></div>
    </section>
    )
}